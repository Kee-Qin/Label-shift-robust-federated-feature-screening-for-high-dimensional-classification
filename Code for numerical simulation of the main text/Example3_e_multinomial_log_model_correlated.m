%% Example3 (d)
% multinomial logistic model feature correlated
...
Sigma = diag(ones(p,1),0) + l1(1:p,1:p)*2/3 + l2(1:p,1:p)*2/3 + l3(1:p,1:p)/3 +l4(1:p,1:p)/3;
mu = zeros(p,1);
alpha_list =  [0.06,0.08,0.1,0.2,1,5,10];
...
for Class =6:7
    ...
    for qq = 1:length(alpha_list)
        ...
        parfor T = 1:iter
            ...
            beta1 = zeros(p,1);
            W = binornd(1,0.5,6,1);
            beta_M = power(-1,W).*(1.5);
            beta1(M) = beta_M;
    
            probability1 = exp(X*beta1-2);
            probability2 = ones(N,1)*0.8;
            ...
            Generate = generation;
            Y1 = Generate.getY(N,m,1,probability_all,Class);
    ...
    if add_noise
        name = "example3(e)_class"+num2str(Class)+"_reg_noise.mat"; 
    else
        name = "example3(e)_class"+num2str(Class)+"_reg.mat"; 
    end
    save(name,"Final")
end