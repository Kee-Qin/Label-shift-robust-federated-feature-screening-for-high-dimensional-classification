...
prob_list = [4,6];
for tt = 1:2
    probability = prob_list(tt);
    Final = [];
    for shuffle_rate = 0:0.05:0.4
        ...
        result = [shuffle_rate;result];
        Final = [Final result];
    end
    
    if add_noise
        name = "example4(f)_class"+num2str(Class)+'_'+num2str(probability)+"_"+num2str(100*miu)+"_noise.mat"; 
    else
        name = "example4(f)_class"+num2str(Class)+'_'+num2str(probability)+"_"+num2str(100*miu)+".mat"; 
    end
    save(name,"Final")
end